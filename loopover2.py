...
if __name__ == "__main__":
	...
	while found:
		...
		newBoards, newCount = split_boards(pool.map(defs2.get_new_positions, newBoards, int(len(newBoards) / multiprocessing.cpu_count()) + 1))
		...
		for board in defs2.InChunks(newBoards,multiprocessing.cpu_count()):
			...
			sames.update(*pool.starmap(defs2.and_them, zip(itertools.repeat(board), defs2.InChunks(boards, multiprocessing.cpu_count())), int(len(newBoards) / multiprocessing.cpu_count()) + 1))
			# same = set().union(*same)
			# sames.update(same)
		newBoards -= sames
